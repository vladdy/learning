...
V3F QuadControl::RollPitchControl(V3F accelCmd, Quaternion<float> attitude, float collThrustCmd)
{
  ...
  // Attitude command
  float collAcceleration = -collThrustCmd / mass;
  float bxCmd = accelCmd.x / collAcceleration;
  float byCmd = accelCmd.y / collAcceleration;

  // Commanded rates of bx and by
  float bxDotCmd = kpBank * (bxCmd - R(0, 2));
  float byDotCmd = kpBank * (byCmd - R(1, 2));
...
float QuadControl::AltitudeControl(float posZCmd, float velZCmd, float posZ, float velZ, Quaternion<float> attitude, float accelZCmd, float dt)
{
  ...
  float u1Bar = kpPosZ * errorPos + kpVelZ * errorVel + KiPosZ * integratedAltitudeError + accelZCmd;

  float c = (u1Bar - CONST_GRAVITY) / R(2, 2);
